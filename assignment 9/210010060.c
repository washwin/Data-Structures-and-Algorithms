...
int main(int argc, char *argv[])
{
  ...
  mergesort(0, elist.size - 1);                 //to sort according to weights

  rewind(in);
  i = 0;
  slist.size = 0;
  while (fscanf(in, "%d %d %d", &x, &y, &z) != EOF)     //make vertices list
  {
    if (search(x) == 210010060)                         //if vertex x is not in the list         
    {
    ...
    if (search(y) == 210010060)                         //if vertex y is not in the list
    {
  ...
  for (i = 0; i < elist.size; i++)                              //implementing Kruskal's algorithm
  {
...
int search(int n)
{
  for (int i = 0; i < slist.size; i++)
  {
    for (int j = 0; j < slist.list[i].size; j++)
    {
      ...
      {
        return i;                         //returns position of vertex in spanlist
      }
    }
  }
  return 210010060;                      //returns false value
}
